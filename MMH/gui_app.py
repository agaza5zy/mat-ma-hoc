...
class HastadApp:
    ...
    def run_rsa_standard(self):
        ...
        try:
            ...
            self.rsa_log.insert(tk.END, f"Khóa công khai (e, N): ({pub[0]}, {pub[1]})\n\n")

            c = self.rsa_system.encrypt(msg, pub)
            self.current_c = c
            self.rsa_log.insert(tk.END, f"Bước 2: Mã hóa thông điệp thành công.\n")
            self.rsa_log.insert(tk.END, f"Bản mã C: {c}\n\n")

            self.rsa_log.insert(tk.END, f"Bước 3: Tấn công khai căn trực tiếp\n")
            
            m_pow_e = m_int ** e
            self.rsa_log.insert(tk.END, f"  - Khảo sát điều kiện m^e so với N:\n")
            self.rsa_log.insert(tk.END, f"      m^e = {m_pow_e}\n")
            self.rsa_log.insert(tk.END, f"      N   = {pub[1]}\n")
            
            if m_pow_e < pub[1]:
                self.rsa_log.insert(tk.END, f"  => KẾT LUẬN: Đủ điều kiện m^e < N. Thực hiện khai căn sẽ thành công!\n")
            else:
                self.rsa_log.insert(tk.END, f"  => KẾT LUẬN: Không đủ điều kiện (m^e >= N). Khai căn sẽ ra kết quả sai.\n")

            m_root = iroot_e(c, e)
            self.rsa_log.insert(tk.END, f"\nGiá trị số nguyên m tính được từ khai căn (kẻ tấn công): {m_root}\n")
            self.rsa_log.insert(tk.END, f"Giá trị số nguyên m gốc ban đầu: {m_int}\n")
            
            if m_root == m_int:
                self.rsa_log.insert(tk.END, f"Kết quả khai căn trùng khớp với bản rõ ban đầu!\n")
                self.rsa_log.insert(tk.END, f"Thông điệp thu được: {m_root.to_bytes((m_root.bit_length()+7)//8, 'big').decode()}\n")
            else:
                self.rsa_log.insert(tk.END, f"Tấn công khai căn THẤT BẠI. Dữ liệu giải mã bị sai lệch.\n")
    ...
    def run_hastad_broadcast(self):
        ...
        try:
            ...
            self.hastad_log.insert(tk.END, f"\nBước 2: Hệ phương trình Định lý số dư Trung Hoa (CRT):\n")
            for i in range(e):
                self.hastad_log.insert(tk.END, f"  X ≡ {ciphertexts[i]} (mod {moduli[i]})\n")
