...
# FIX-4: Import thêm hàm attack_direct_iroot_verbose từ file backend
from test_hastad_attack import RSAModule, chinese_remainder_theorem, iroot_e, hastad_broadcast_attack, generate_rsa_components, attack_direct_iroot_verbose
...
class HastadApp:
    def __init__(self, root):
        ...
        # FIX-6: Điều chỉnh key_size mặc định khởi tạo ban đầu cho đồng bộ
        self.rsa_system = RSAModule(key_size=128)
        
        # Lưu trữ trạng thái tạm thời cho Tab 1
        self.current_priv = None
        self.current_c = None
    ...
    def _setup_tab_rsa(self):
        ...
        self.rsa_msg.insert(0, "Crypto2026")
        ...
        btn_enc = tk.Button(frame_btn, text="Sinh khóa & Mã hóa", command=self.run_rsa_standard, font=("Helvetica", 13, "bold"), bg="#98c379", fg="#1e222b", activebackground="#a3e08a", bd=0, padx=15, pady=6)
        btn_enc.pack(side="left", padx=5)

        btn_dec = tk.Button(frame_btn, text="Giải mã (Khóa bí mật d)", command=self.run_rsa_decrypt, font=("Helvetica", 13, "bold"), bg="#e5c07b", fg="#1e222b", activebackground="#f3d498", bd=0, padx=15, pady=6)
    ...
    # FIX-4: Cập nhật hàm xử lý thông tin mã hóa, giải mã thông thường và xuất nhật ký tấn công chi tiết
    def run_rsa_standard(self):
        ...
        try:
            ...
            # Tạo cặp khóa RSA và lưu trữ khóa bí mật cho bước giải mã hợp lệ
            pub, priv = self.rsa_system.generate_keys(e_custom=e)
            self.current_priv = priv
            
            # Thực hiện mã hóa lấy bản mã c
            c = int(self.rsa_system.encrypt(msg, pub))
            self.current_c = c

            # Giải mã thông thường bằng khóa bí mật d
            decrypted_bytes = self.rsa_system.decrypt(c, priv)
            decrypted_value = int.from_bytes(decrypted_bytes, 'big')

            # Gọi hàm phân tích nghiệp vụ tấn công khai căn từ backend
            result = attack_direct_iroot_verbose(c, e, pub[1])

            # Kết xuất log ra màn hình UI theo đúng thứ tự biểu diễn của Task yêu cầu
            self.rsa_log.insert(tk.END, "=== BƯỚC MÃ HÓA ===\n")
            self.rsa_log.insert(tk.END, f"  m (số nguyên): {m_goc_int}\n")
            self.rsa_log.insert(tk.END, f"  e: {e},  n: {pub[1]}\n")
            self.rsa_log.insert(tk.END, f"  c = m^e mod n = {c}\n\n")
            
            self.rsa_log.insert(tk.END, "=== BƯỚC GIẢI MÃ BÌNH THƯỜNG (dùng khóa bí mật d) ===\n")
            self.rsa_log.insert(tk.END, f"  m_giải_mã = c^d mod n = {decrypted_value}\n\n")
            
            self.rsa_log.insert(tk.END, "=== BƯỚC TẤN CÔNG KHAI CĂN ===\n")
            self.rsa_log.insert(tk.END, "  Kiểm tra điều kiện: m^e < n ?\n")
            self.rsa_log.insert(tk.END, f"  c = {result['m_pow_e_raw']}\n")
            self.rsa_log.insert(tk.END, f"  iroot(c, e) = {result['root']}\n")
            self.rsa_log.insert(tk.END, f"  Khai căn chính xác (exact): {result['exact']}\n")
            self.rsa_log.insert(tk.END, f"  Xác minh: root^e mod n = {result['root_repow_mod_n']}\n")
            
            if result['success']:
                try:
                    decoded_text = result['root'].to_bytes((result['root'].bit_length() + 7) // 8, 'big').decode('utf-8')
                except Exception:
                    decoded_text = str(result['root'])
                self.rsa_log.insert(tk.END, f"  ✅ Tấn công thành công! Bản rõ: {decoded_text}\n")
            else:
                self.rsa_log.insert(tk.END, "  ❌ Tấn công thất bại — m^e đã bị rút modulo\n")

        except Exception as ex:
            messagebox.showerror("Lỗi", str(ex))

    def run_rsa_decrypt(self):
        if not self.current_priv or not self.current_c:
            messagebox.showwarning("Cảnh báo", "Vui lòng 'Sinh khóa & Mã hóa' trước khi giải mã hợp lệ!")
            return
        
        try:
            self.rsa_log.insert(tk.END, f"\n{'-'*50}\n")
            self.rsa_log.insert(tk.END, f"Bước Giải mã hợp lệ (Người nhận dùng khóa bí mật d):\n")
            decrypted_msg = self.rsa_system.decrypt(self.current_c, self.current_priv)
            self.rsa_log.insert(tk.END, f"Thông điệp giải mã thành công: {decrypted_msg.decode('utf-8')}\n")
        except Exception as ex:
            messagebox.showerror("Lỗi Giải Mã", str(ex))
    ...
    def _setup_tab_hastad(self):
        ...
        self.hastad_msg.insert(0, "Hello CRT")
    ...
    # FIX-5 & FIX-6: Điều chỉnh quy trình sinh cấu phần RSA bằng generate_rsa_components (512-bit) và hiển thị chi tiết CRT
    def run_hastad_broadcast(self):
        ...
        try:
            ...
            # FIX-6: Thay thế vòng lặp generate_keys bằng generate_rsa_components để đồng bộ mã nguồn 512-bit
            m_expected, ciphertexts, moduli = generate_rsa_components(msg, e, key_size=512)
            
            for i in range(e):
                self.hastad_log.insert(tk.END, f"Người nhận {i+1}:\n  C = {ciphertexts[i]}\n  N = {moduli[i]}\n")
            
            # FIX-5: Bổ sung in từng phương trình đồng dư dạng M ≡ c_i (mod n_i) và Tích số chung lớn Ne
            self.hastad_log.insert(tk.END, "\nHệ phương trình đồng dư cần giải (CRT):\n")
            for i, (c_i, n_i) in enumerate(zip(ciphertexts, moduli)):
                self.hastad_log.insert(tk.END, f"  M ≡ {c_i} (mod {n_i})\n")
                
            tich_moduli = reduce(lambda a, b: a * b, moduli)
            self.hastad_log.insert(tk.END, f"\nTích N1*N2*...*Ne = {tich_moduli}\n")
            
            # Giải hệ CRT thu về m^e nguyên bản
            m_pow_e = chinese_remainder_theorem(ciphertexts, moduli)
            self.hastad_log.insert(tk.END, f"\n=> Giải hệ phương trình, ta được X (chính là m^{e} mod (N1*N2*...)): \n{m_pow_e}\n\n")
            ...
            self.hastad_log.insert(tk.END, f"\nBản rõ suy ra từ các bước trên: {plain.decode('utf-8', errors='ignore')}\n")
        except Exception as ex:
            messagebox.showerror("Lỗi", str(ex))
    ...
    def _setup_tab_visual(self):
        ...
        self.visual_msg_nho.insert(0, "A")
        ...
        self.visual_msg_lon.insert(0, "ThongDiepSieuDaiNhungPhaiDuoi40KyTu")
        ...
        self.visual_msg_crt.insert(0, "Test CRT Match")
    ...
    def run_compare_e(self):
        ...
        try:
            _, c_list, n_list = generate_rsa_components(m_bytes, 3, key_size=128)
            for i in range(3):
